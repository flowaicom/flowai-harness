...
/**
 * The 13 distinct part types of the AI SDK Data Stream Protocol.
 *
...
const VALID_STREAM_PART_TYPES: ReadonlySet<string> = new Set<StreamPart["type"]>([
  "text",
  "reasoning",
  "step-start",
  "tool-invocation",
...
export const parseSSELine = (line: string): StreamPart | null => {
  ...
  try {
    ...
    if (!VALID_STREAM_PART_TYPES.has(parsed.type)) return null;
    return parsed as StreamPart;
  } catch {
    return null;
  }
};
