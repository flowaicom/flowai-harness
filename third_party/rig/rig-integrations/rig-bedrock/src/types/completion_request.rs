...
use aws_sdk_bedrockruntime::types::{
    CachePointBlock, CachePointType, ContentBlock, InferenceConfiguration, SystemContentBlock,
...
impl AwsCompletionRequest {
    pub fn new(request: rig::completion::CompletionRequest, prompt_caching: bool) -> Self {
        Self {
            request,
            prompt_caching,
        }
    ...
    fn cache_point() -> Result<CachePointBlock, CompletionError> {
    ...
    pub fn inference_config(&self) -> Option<InferenceConfiguration> {
        ...
        if let Some(max_tokens) = &self.request.max_tokens {
            inference_configuration =
    ...
    pub fn tools_config(&self) -> Result<Option<ToolConfiguration>, CompletionError> {
        let mut tools = vec![];
        for tool_definition in self.request.tools.iter() {
        ...
        if !tools.is_empty() {
            if self.prompt_caching {
                tools.push(Tool::CachePoint(Self::cache_point()?));
            }

            // Convert rig's ToolChoice to AWS Bedrock ToolChoice
            use aws_sdk_bedrockruntime::types as aws_bedrock;
            let tool_choice = self.request.tool_choice.as_ref().and_then(|choice| {
                match choice {
    ...
    pub fn system_prompt(&self) -> Result<Option<Vec<SystemContentBlock>>, CompletionError> {
        ...
        if let Some(system_prompt) = self.request.preamble.to_owned()
            && !system_prompt.is_empty()
        {
            system_blocks.push(SystemContentBlock::Text(system_prompt));
        }

        for message in self.request.chat_history.iter() {
        ...
        if system_blocks.is_empty() {
            ...
            if self.prompt_caching {
                system_blocks.push(SystemContentBlock::CachePoint(Self::cache_point()?));
            }
    ...
    pub fn messages(&self) -> Result<Vec<aws_bedrock::Message>, CompletionError> {
        ...
        if !self.request.documents.is_empty() {
            let messages = self
                .request
                .documents
                .iter()
        ...
        self.request.chat_history.iter().for_each(|message| {
            ...
            && !last_message.content.is_empty()
        {
            last_message
                .content
                .push(ContentBlock::CachePoint(Self::cache_point()?));
        }
...
mod tests {
    ...
    fn test_tool_choice_auto_conversion() {
        ...
        let aws_request = AwsCompletionRequest::new(request, false);
    ...
    fn test_system_prompt_includes_system_history() {
        ...
        let aws_request = AwsCompletionRequest::new(request, false);
        let system_prompt = aws_request.system_prompt().expect("system prompt");
    ...
    #[test]
    fn test_prompt_caching_appends_tool_cache_point() {
        let request = CompletionRequest {
            model: None,
            tools: vec![ToolDefinition {
        ...
        let aws_request = AwsCompletionRequest::new(request, true);
        let tool_config = aws_request
            .tools_config()
            .expect("Should build tool config")
            .expect("tool config");
    ...
    #[test]
    fn test_prompt_caching_appends_system_cache_point() {
        let request = CompletionRequest {
            model: None,
    ...
    #[test]
    fn test_prompt_caching_appends_message_cache_point() {
        let request = CompletionRequest {
            model: None,
            chat_history: OneOrMany::one(Message::User {
                content: OneOrMany::one(UserContent::Text(Text {
    ...
    #[test]
    fn test_prompt_caching_covers_tools_system_and_messages_together() {
        let request = CompletionRequest {
            model: None,
            preamble: Some("system".to_string()),
            tools: vec![ToolDefinition {
