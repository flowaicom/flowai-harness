...
pub struct Usage {
    ...
    /// The number of cached input read tokens (from prompt caching). 0 if not reported by provider.
    pub cached_input_tokens: u64,
    /// The number of cache-write input tokens (prompt-cache population). 0 if not reported by provider.
