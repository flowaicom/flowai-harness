...
impl TryFrom<CompletionResponse> for completion::CompletionResponse<CompletionResponse> {
    ...
    fn try_from(response: CompletionResponse) -> Result<Self, Self::Error> {
        ...
        let cache_read_input_tokens = response.usage.cache_read_input_tokens();
        let cache_creation_input_tokens = response.usage.cache_creation_input_tokens();
...
/// Apply cache control to tool definitions by placing a single breakpoint on the
/// last tool definition. Anthropic caches the tool prefix through the marked
/// element, so clearing earlier markers avoids fragmented cache prefixes.
pub fn apply_tools_cache_control(tools: &mut [ToolDefinition]) {
...
/// Apply cache control to the final serialized tool list so mixed tool sources
/// (`CompletionRequest.tools` plus provider-native `additional_params.tools`)
/// share a single breakpoint on the last effective tool definition.
pub fn apply_tool_values_cache_control(tools: &mut [serde_json::Value]) {
    for tool in tools.iter_mut() {
...
impl TryFrom<AnthropicRequestParams<'_>> for AnthropicCompletionRequest {
    ...
    fn try_from(params: AnthropicRequestParams<'_>) -> Result<Self, Self::Error> {
        ...
        if prompt_caching {
            apply_tool_values_cache_control(&mut tools);
        }
...
mod tests {
    ...
    fn test_cache_control_serialization() {
        ...
        assert!(tool_json.contains(r#""cache_control":{"type":"ephemeral"}"#));
        ...
        assert_eq!(tools[0].cache_control, None);
        ...
        apply_tool_values_cache_control(&mut tool_values);
        assert!(tool_values[0].get("cache_control").is_none());
        assert_eq!(
            tool_values[1].get("cache_control"),
            Some(&serde_json::json!({ "type": "ephemeral" }))
        );
    }

    #[test]
    fn test_prompt_caching_marks_last_tool_after_merging_additional_tools() {
        let request = CompletionRequest {
            model: None,
            preamble: Some("system".to_string()),
            chat_history: OneOrMany::one(crate::message::Message::user("hi")),
            documents: vec![],
            tools: vec![crate::completion::ToolDefinition {
                name: "framework_tool".to_string(),
                description: "Framework tool".to_string(),
            ...
            additional_params: Some(serde_json::json!({
                "tools": [
                    {
                        "name": "provider_tool",
                        "description": "Provider-native tool",
        ...
        assert_eq!(
            request.tools[1].get("cache_control"),
            Some(&serde_json::json!({ "type": "ephemeral" }))
        );
    }
    ...
    #[test]
    fn test_usage_reports_cache_reads_and_writes_as_cached_input_tokens() {
        let usage = Usage {
            input_tokens: 100,
        ...
        let converted = usage.token_usage().expect("usage conversion");
        assert_eq!(converted.input_tokens, 100);
        assert_eq!(converted.cached_input_tokens, 50);
        assert_eq!(converted.total_tokens, 190);
    }

    #[test]
    fn test_completion_response_preserves_cache_reads_and_writes_in_usage() {
        ...
        let converted: completion::CompletionResponse<CompletionResponse> =
            response.try_into().expect("response conversion");
        assert_eq!(converted.usage.input_tokens, 100);
        assert_eq!(converted.usage.cached_input_tokens, 50);
        assert_eq!(converted.usage.total_tokens, 190);
    }
}
